<?php
namespace library\EuFrameTemp;
use library\EuFrameInc\EUInc;
use library\EuFrameData\EUData;
use library\EuFrameLang\EULang;
use library\EuFrameTree\EUTree;
...
class EUTemp{
    ...
    function TempReplace($content){
        ...
        $replacement=array(
        '<?php if("${1}"=="null"):if(rtrim(library\EuFrameInc\EUInc::CurPageUrl(),"/")==rtrim($GLOBALS["config"]["APPURL"],"/")):echo"${2}";endif;else:if(library\EuFrameInc\EUInc::Contain("${1}",library\EuFrameInc\EUInc::CurPageUrl())):echo"${2}";endif;endif;?>',
		...
		'<?php if(library\EuFrameInc\EUInc::Contain(",","${1}")):$pluginfile=explode(",","${1}");$HOOKPATH=APP_ROOT."/plugins/$pluginfile[0]/";if(is_dir($HOOKPATH)):if(library\EuFrameInc\EUInc::Contain(".php","$pluginfile[1]")):include_once $HOOKPATH.$pluginfile[1];else:echo"<iframe src=$HOOKPATH.$pluginfile[1] frameborder=0 id=external-frame></iframe><style>iframe{width:100%;margin:0 0 1em;border:0;}</style><script src=assets/js/aEUoheight.js></script>";endif;endif;else:$HOOKPATH=APP_ROOT."/plugins/${1}/";if(is_dir($HOOKPATH)):include_once $HOOKPATH."index.php";endif;endif;?>',
        ...
        '<?php echo library\EuFrameInc\EUInc::CEUSubstr(library\EuFrameInc\EUInc::DeleteHtml($this->tplvars["${1}"]["${2}"]),${3},${4}); ?>',
		'<?php echo library\EuFrameInc\EUInc::CEUSubstr(library\EuFrameInc\EUInc::DeleteHtml($this->tplvars["${1}"]),${2},${3}); ?>',
		...
		'<?php $tree=new library\EuFrameTree\EUTree();$tree->Init($this->tplvars["${2}"]);if(${1}==0):echo$tree->SubClass(${3},${4});elseif(${1}==1):$string="<option value=\\\$id \\\$selected \\\$disabled>\\\$spacer\\\$name</option>";echo$tree->GetTree(0,$string,${3},${4});elseif(${1}==2):$url=library\EuFrameInc\EUInc::ClearParam("id",library\EuFrameInc\EUInc::ClearParam("do",$_SERVER["QUERY_STRING"]));$string="<div class=row style=margin-bottom:15px;font-size:14px;><div class=col-9 data-id=\\\$id data-name=\\\$name>\\\$spacer\\\$name</div><div class=col-3><a id=\'tree-mod\' class=\'mr-2\' href=?".$url."&id=\\\$id&do=mon>编辑</a> <a id=\'tree-del\' href=?".$url."&id=\\\$id&do=del>删除</a></div></div>";echo$tree->GetTree(0,$string,${3});endif;?>',
        '<?php echo library\EuFrameLang\EULang::LangSet("${1}",${2});?>',
        '<?php echo library\EuFrameLang\EULang::LangData("${1}");?>',
		'<?php echo library\EuFrameLang\EULang::LangData($this->tplvars["${1}"]["${2}"]);?>',
		'<?php if(library\EuFrameInc\EUInc::Contain(",","${1}")):$langdata=explode(",","${1}");echo library\EuFrameLang\EULang::ModLangData($langdata[0],$langdata[1]);else:echo library\EuFrameLang\EULang::ModLangData("${1}");endif;?>',
		'<?php echo library\EuFrameLang\EULang::ModLangData($this->tplvars["${1}"]["${2}"]);?>',
		'<?php $Page=new library\EuFramePage\EUPage($this->tplvars["${1}"],$this->tplvars["${2}"],$this->tplvars["${3}"],$this->tplvars["${4}"],2);echo$Page->ShowPager();?>',
		'<?php $Page=new library\EuFramePage\EUPage($this->tplvars["${1}"],$this->tplvars["${2}"],$this->tplvars["${3}"],$this->tplvars["${4}"],${5});echo$Page->ShowPager();?>',
        ...
        $content=preg_replace_callback(
            ...
            function($matches){
                if(EUInc::Contain("inc",$matches[1])){
                    return $this->StripTags("<?php echo library\EuFrameInc\EUInc::$matches[2];?>");
                }elseif(EUInc::Contain("db",$matches[1])){
                    return $this->StripTags("<?php echo library\EuFrameData\EUData::$matches[2];?>");
                }elseif(EUInc::Contain("url",$matches[1])){
                    return $this->StripTags("<?php echo library\EuFrameRoEUe\EURoEUe::$matches[2];?>");
                }
            },
            $content
        );
