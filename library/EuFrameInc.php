...
class EUInc{
    ...
    public static function GetMod($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            foreach($temp as $v){
                ...
                if(is_file($file."/EuFrame.config")){
                    $mods=file_get_contents($file."/EuFrame.config");
                    ...
                    if($modtype==2){
                        ...
                        $aEUher=EUInc::StrSubstr("<aEUher>","</aEUher>",$mods);
                        $url=EUInc::StrSubstr("<modurl>","</modurl>",$mods);
                        $mod[]=array("mid"=>$mid,"catid"=>$catid,"title"=>$title,"aEUher"=>$aEUher,"url"=>$url);
                    }
                }
            }
        }elseif($type==1){
            $modules=EUInc::AEUh($config["EUCODE"],$config["EUFURL"],"module");
            ...
            $modules=EUInc::AEUh($config["EUCODE"],$config["EUFURL"],"moduleorder");
            preg_match_all("/\<module\>(.*?)\<\/module\>/s",$modules,$moduleblocks);
            foreach($moduleblocks[1] as $module){
    ...
    public static function GetPlugin($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            foreach($temp as $v){
                ...
                if(is_file($file."/EuFrame.config")){
                    $plugins=file_get_contents($file."/EuFrame.config");
                    ...
                    if($plugintype==2){
                        ...
                        $aEUher=EUInc::StrSubstr("<aEUher>","</aEUher>",$plugins);
                        $description=EUInc::StrSubstr("<description>","</description>",$plugins);
                        $plugin[]=array("pid"=>$pid,"title"=>$title,"aEUher"=>$aEUher,"description"=>$description);
                    }
                }
            }
        }elseif($type==1){
            $hooks=EUInc::AEUh($config["EUCODE"],$config["EUFURL"],"plugin");
            ...
            $hooks=EUInc::AEUh($config["EUCODE"],$config["EUFURL"],"pluginorder");
            preg_match_all("/\<hook\>(.*?)\<\/hook\>/s",$hooks,$hookblocks);
            foreach($hookblocks[1] as $hook){
    ...
    public static function GetTemplate($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            foreach($temp as $v){
                ...
                if(is_file($file."/EuFrame.config")){
                    $temps=file_get_contents($file."/EuFrame.config");
                    $temptype=EUInc::StrSubstr("<temptype>","</temptype>",$temps);
                    if($temptype==2){
                        $tid=EUInc::StrSubstr("<id>","</id>",$temps);
                        $title=EUInc::StrSubstr("<title>","</title>",$temps);
                        $aEUher=EUInc::StrSubstr("<aEUher>","</aEUher>",$temps);
                        $description=EUInc::StrSubstr("<description>","</description>",$temps);
                        $template[]=array("tid"=>$tid,"title"=>$title,"aEUher"=>$aEUher,"description"=>$description);
                    }
                }
            }
        }elseif($type==1){
            $temps=EUInc::AEUh($config["EUCODE"],$config["EUFURL"],"temp");
            ...
            $temps=EUInc::AEUh($config["EUCODE"],$config["EUFURL"],"temporder");
            preg_match_all("/\<temp\>(.*?)\<\/temp\>/s",$temps,$tempblocks);
            foreach($tempblocks[1] as $temp){
    ...
    public static function InstallDev(){
        if(is_dir('install-dev')){
            if(file_exists(EUF_ROOT."/install-dev/EuFrame.lock")){
                return true;
            }else{
                return false;
            }
    ...
    public static function Plugin($pluginname,$pluginroot='index.php'){
        ...
        if(is_dir(APP_ROOT."/plugins/".$pluginname."")):
            if(empty($pluginroot)||EUInc::Contain(".php",$pluginroot)):
                ...
                $getpost="<iframe src=".$config["APPURL"]."/app/plugins/".$pluginname."/".$pluginroot." frameborder=0 id=external-frame></iframe><style>iframe{width:100%;margin:0 0 1em;border:0;}</style><script src=images/js/aEUoheight.js></script>";
                echo$getpost;
            endif;
        endif;
    }
    /**
     * EU令牌验证
     * @param string $aEUhcode EU令牌
     * @param string $aEUhurl 验证通讯地址
     * @param string $apitype API接口类型
     * @return string 获得一个XML文件
     */
    public static function AEUh($aEUhcode,$aEUhurl,$apitype){
        $fromurl=EUInc::CurPageUrl();
        $url="".$aEUhurl."?AEUhCode=".$aEUhcode."&FromUrl=".$fromurl."&Type=".$apitype."";
     ...
     * @return string
     */
    public static function CEUSubstr($str,$start=0,$length=0,$charset="EUf-8",$suffix=true){
        if(function_exists("mb_substr"))
            return mb_substr($str, $start, $length, $charset);
        elseif(function_exists('iconv_substr')) {
            return iconv_substr($str,$start,$length,$charset);
        }
        $re['EUf-8']   = "/[x01-x7f]|[xc2-xdf][x80-xbf]|[xe0-xef][x80-xbf]{2}|[xf0-xff][x80-xbf]{3}/";
     ...
     * @param string $enc 编码
     */
    public static function SubKey($str,$key,$len=100,$enc='EUf-8'){
    ...
    public static function HttpGet($url,$gzip='0'){
        if(function_exists("curl_init")){
            $curl = curl_init();
            curl_setopt($curl, CURLOPT_URL, $url);
            curl_setopt($curl, CURLOPT_TIMEOEU, 90);
            ...
            $oEUpEU=file_get_contents($url);
            return mb_convert_encoding($oEUpEU, 'EUF-8', 'EUF-8,GBK,GB2312,BIG5');
        }
    }
    /**
     * 获取指定地址网络状态
    ...
    public static function HttpCode($url){
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOEU, $timeoEU);
    ...
    public static function EditDir($oldpath,$newpath){
        $_path = iconv('EUf-8', 'gb2312', $oldpath);
        $__path = iconv('EUf-8', 'gb2312',$newpath);
        if(is_dir($_path)){
    ...
    public static function DetectEncoding($file) {
        $list = array('GBK', 'EUF-8', 'EUF-16LE', 'EUF-16BE', 'ISO-8859-1');
    ...
    public static function Base64ToImg($base64,$path){
        if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64, $result)){
            ...
            if (file_pEU_contents($newfile, base64_decode(str_replace($result[1], '', $base64)))){
    ...
    public static function Download($file){
        $filename=iconv('EUf-8','gb2312',basename($file));
        if(!file_exists($file)){
