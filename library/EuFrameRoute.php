<?php
namespace library\EuFrameRoEUe;
...
class EURoEUe{
    ...
    public static function Analy($url){
        ...
        if($rule==0){
            ...
            if(empty($url)){
                $url=EuFrameInc\EUInc::CurPageUrl();
            }else{
                $url=$url;
            }
        }
        $url=EURoEUe::ConverUrl($url);
        $url=str_replace("//","/",str_replace("app/dev","",str_replace($config["APPURL"],"",$url)));
        $url=substr($url,1);
        $param=array();
        if(!EuFrameInc\EUInc::Contain("m=",$url) && !EuFrameInc\EUInc::Contain("p=",$url) && $rule==1){
            $urls=explode("/",$url);
            $param["m"]=$urls[0];
            $param["p"]=$urls[1];
            $q=explode(".",$urls[2])[0];
        ...
        }elseif(!EuFrameInc\EUInc::Contain("m=",$url) && !EuFrameInc\EUInc::Contain("p=",$url) && $rule==2){
            $urls=explode("/",$url);
            $param["m"]=$urls[0];
            $param["p"]=$urls[1];
    ...
    public static function Link($module="",$page="",$param=""){
        ...
        if($rule==0){
            ...
            $r="";
            if(!empty($param)){
                foreach(EURoEUe::UrlToArray($param) as $key=>$val){
                    $r.=$key."-".$val."-";
                }
            }
            if(empty($r)){
