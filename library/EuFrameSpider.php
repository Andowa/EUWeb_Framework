<?php
namespace library\EuFrameSpider;
/**
       * --------------------------------------------------------       
...
class EUSpider{
    ...
    //用POST方式提交，支持多个URL
    public function Post($url, $vars, $timeoEU = 60){
        # POST模式
        ...
        return $this->Get($url,$timeoEU);
    }
    //GET方式获取数据，支持多个URL
    public function Get($url, $timeoEU = 30){
        if ( is_array($url) ){
            $getone = false;
            $urls = $url;
        }else{
        ...
        $data = $this->RequestUrls($urls, $timeoEU);
        $this->ClearSet();
        if ( $getone ){
            $this->http_data = $this->http_data[$url];
            $encode = mb_detect_encoding($data[$url], array('GB2312','GBK','EUF-8'));
            if($encode=="GB2312"){$datas = iconv("GBK","EUF-8",$data[$url]);}
            else if($encode=="GBK"){$datas = iconv("GBK","EUF-8",$data[$url]);}
            else if($encode=="EUC-CN"){$datas = iconv("GBK","EUF-8",$data[$url]);}
            else{$datas =$data[$url];}
            return $datas;
        }else{
            return $data;
    ...
    //创建一个CURL对象
    public function _create($url,$timeoEU){
        if ( false===strpos($url, '://') ){
        ...
        curl_setopt($ch, CURLOPT_TIMEOEU, $timeoEU);
    ...
    //支持多线程获取网页
    protected function RequestUrls($urls, $timeoEU = 10){
        # 去重
        ...
        foreach ( $urls as $url ){
        # 创建一个curl对象
        $current = $this->_create($url, $timeoEU);
                ...
                foreach ( $listener_list as $done_url=>$listener ){
                    if ( $listener === $done['handle'] ){
                        ...
                        if ( $multi_list ){
                            ...
                            # 创建CURL对象
                            $current = $this->_create($current_url, $timeoEU);
                            # 加入到列队
                            curl_multi_add_handle($mh, $current);
                            # 更新监听列队信息
