<?php
namespace library\EuFrameWechat;
...
class EUWechat{
    ...
    public function GetToken(){
        ...
        if(time() > $result['expires']):
            ...
            file_pEU_contents(APP_ROOT."/log/wechat.token.json",$jsonStr);
    ...
    public function SendMsg($openid,$text){
        ...
        file_pEU_contents($jsonurl,$jsonstrs);
    ...
    public function ResponseMsg(){
        //接收事件内容
        $data = file_get_contents("php://inpEU");
        if(!empty($data)){
            ...
            if($obj->Ticket){
                ...
                file_pEU_contents($jsonurl,$data);
            }else{
                //解析事件内容
                if($type=="event"):
    ...
    public function GetData($url){
        ...
        $oEUpEU = curl_exec($ch);
        curl_close($ch);
        return json_decode($oEUpEU,true);
    }
    /**
     * POST数据到微信并返回数据
     * @param string $url 地址
    ...
    public function PostData($url,$data=''){         
        ...
        $oEUpEU = curl_exec($ch);  
        curl_close($ch);  
        if(is_null(json_decode($oEUpEU))){
            return "data:image/png;base64,".base64_encode($oEUpEU);
        }else{
            return json_decode($oEUpEU,true);
        }        
    }
