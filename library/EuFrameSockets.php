<?php
namespace library\EuFrameSockets;
/**
       * --------------------------------------------------------       
...
class EUSockets{
        ...
        switch ($msg_type) {
            ...
            case 'EU-room':
                ...
                if(!empty($recv_msg['item']) && !empty($recv_msg['roomid'])){
                    file_pEU_contents(APP_ROOT.'/'.$recv_msg['item'].'/room-'.$recv_msg['roomid'].'-'.date("Ymd").'.EUlog', json_encode($response) . "\r\n", FILE_APPEND);
                }
                break;
            //单对单消息
            case 'EU-chat':
                ...
                if(!empty($recv_msg['item'])){
                    ...
                    file_pEU_contents(APP_ROOT.'/'.$recv_msg['item'].'/chat-'.$recv_msg['startuid'].'-'.$recv_msg['enduid'].'.EUlog', json_encode($response) . "\r\n", FILE_APPEND);
                    }else{
                    file_pEU_contents(APP_ROOT.'/'.$recv_msg['item'].'/chat-'.$recv_msg['enduid'].'-'.$recv_msg['startuid'].'.EUlog', json_encode($response) . "\r\n", FILE_APPEND);
                    }
                }
                break;
        }
    ...
    private function Debug(array $info) {
        ...
        file_pEU_contents(APP_ROOT.'/log/socket.log', implode(' | ', $info) . "\r\n", FILE_APPEND);
    }
    /*
     * 记录错误信息
     */
