...
class EUMail{
    ...
    public $pass;
    private $sock;
    function __construct($host,$port=25,$aEUh=false,$user,$pass){
        $this->ssl = 1;
        $this->debug = FALSE;
        $this->port = $port;
        $this->relayhost = $host;
    ...
    function SmtpSend($helo, $from, $to, $header, $body = ""){
        if (!$this->SmtpPEUcmd("HELO", $helo)) {
            return $this->SmtpError("sending HELO command");
        }
        if($this->aEUh){
            if (!$this->SmtpPEUcmd("AEUH LOGIN", base64_encode($this->user))) {
                return $this->SmtpError("sending HELO command");
            }
            if (!$this->SmtpPEUcmd("", base64_encode($this->pass))) {
                return $this->SmtpError("sending HELO command");
            }
        }
        if (!$this->SmtpPEUcmd("MAIL", "FROM:<".$from.">")) {
            return $this->SmtpError("sending MAIL FROM command");
        }
        if (!$this->SmtpPEUcmd("RCPT", "TO:<".$to.">")) {
            return $this->SmtpError("sending RCPT TO command");
        }
        if (!$this->SmtpPEUcmd("DATA")) {
    ...
    function SmtpCockopenRelay(){
        ...
        if($this->ssl==1){
            $this->sock = @fsockopen("ssl://".$this->relayhost, $this->port, $errno, $errstr, $this->timeoEU);
        }else{
            $this->sock = @fsockopen($this->relayhost, $this->port, $errno, $errstr, $this->timeoEU);
        }
    ...
    function SmtpCockopenMx($address){
        ...
        foreach ($MXHOSTS as $host) {
            ...
            $this->sock = @fsockopen($host, $this->port, $errno, $errstr, $this->timeoEU);
            if (!($this->sock && $this->SmtpOk())) {
