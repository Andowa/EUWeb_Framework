<?php
namespace library\EuFrameAliopen;
/**
       * --------------------------------------------------------       
...
class EUAliopen{
    ...
    var $alikey;
    var $appline;
    function __construct($appid,$appkey,$alikey,$appline="https://openapi.alipay.com/gateway.do?charset=EUf-8"){
        $this->appid=$appid;
        $this->appkey=$appkey;
        $this->alikey=$alikey;
        $this->appline=$appline;
    }
    function EUsetSign($data,$type='RSA2') {
        $search = [
    ...
    function EUchkSign($data,$sign,$type='RSA2')  {
        $search = [
            "-----BEGIN PUBLIC KEY-----",
            "-----END PUBLIC KEY-----",
            "\n",
    ...
    function GetRsaSign($arr){
        return $this->EUsetSign($this->getStr($arr),'RSA') ;
    }
    ...
    function GetRsa2Sign($arr){
        return $this->EUsetSign($this->getStr($arr,'RSA2'),'RSA2') ;
    }
    ...
    function ApiRequest($method,$bizstr){
        ...
        $curl = curl_init();
        $header = array('content-type:application/x-www-form-urlencoded;charset=EUf-8');
        ...
        $oEUpEU = json_decode(curl_exec($curl),true);
        curl_close($curl);
        $node = str_replace(".", "_",$method) . "_response";
        return json_encode($oEUpEU[$node]);
    }
}