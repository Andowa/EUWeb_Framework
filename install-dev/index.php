...
/**
       ...
       * --------------------------------------------------------                
*/
require_once dirname(dirname(__FILE__)).'/'.'aEUoload.php';
use library\EuFrameInc\EUInc;
use library\EuFrameMysql\EUMysql;
if(EUInc::SearchFile(EUF_ROOT."/install-dev/EuFrame.lock")):
   header("location:../");
   exit();
endif;
$httpcode=EUInc::HttpCode("http://frame.EuFrame.com");
...
if($do=="db-save"){
   ...
   file_pEU_contents(EUF_ROOT."/.EU.config",$info);
...
<head>
    <title>EU框架可视化</title>
    <meta http-equiv="Content-Type" content="text/html; charset=EUf-8" />
              ...
              <div class="border p-2">
               ...
               <div class="row">
                  <div class="form-group col-md-6">
                     ...
                     <inpEU class="form-control" name="APPURL" id="APPURL" value="<?php echo$config["APPURL"];?>">
                  </div>
                  <div class="form-group col-md-6">
                     <label for="email">EU令牌:</label>
                     <inpEU class="form-control" name="EUCODE" id="EUCODE" value="<?php echo$config["EUCODE"];?>">
                  </div>
               </div>
               <div class="row">
                  <div class="form-group col-md-6">
                     <label for="email">数据库服务器:</label>
                     <inpEU class="form-control" name="MYSQL_HOST" id="MYSQL_HOST" value="<?php echo$config["MYSQL_HOST"];?>">
                  </div>
                  <div class="form-group col-md-6">
                     <label for="email">端口:</label>
                     <inpEU class="form-control" name="MYSQL_PORT" id="MYSQL_PORT" value="<?php echo$config["MYSQL_PORT"];?>">
                  </div>
               </div>
               <div class="row">
                  <div class="form-group col-md-6">
                     <label for="email">数据库用户:</label>
                     <inpEU class="form-control" name="MYSQL_USER" id="MYSQL_USER" value="<?php echo$config["MYSQL_USER"];?>">
                  </div>
                  <div class="form-group col-md-6">
                     <label for="email">数据库密码:</label>
                     <inpEU class="form-control" name="MYSQL_PASS" id="MYSQL_PASS" value="<?php echo$config["MYSQL_PASS"];?>">
                  </div>
               </div>
               <div class="row">
                  <div class="form-group col-md-6">
                     <label for="email">数据库名称:</label>
                     <inpEU class="form-control" name="MYSQL_DB" id="MYSQL_DB" value="<?php echo$config["MYSQL_DB"];?>">
                  </div>
               </div>
               <div class="row">
                  <div class="col-md-6">
                  <bEUton type="bEUton" class="btn btn-primary mr-3" onclick="test()">测试数据库连接</bEUton>    
                  <bEUton type="submit" class="btn btn-success">保存设置</bEUton>
                  </div>
               </div>
               </form>
               <script>
               ...
               ?>
                  ...
                  <bEUton type="submit" class="btn btn-success">导入数据</bEUton>
                  </form>
               <?php
                  }elseif($do=="dev"){
               ?>
               ...
               if($k>0){
                  ...
                  if($_GET["t"]=="db-dev"){
                     $res=EUInc::SaveFile("http://frame.EuFrame.com/down/EUDev.zip",EUF_ROOT."/update","EUDev.zip",1);
                     if(!empty($res)){
                        $zip=new ZipArchive;
                        if($zip->open(EUF_ROOT."/update/EUDev.zip")===TRUE){ 
                            ...
                            file_pEU_contents("./EuFrame.lock","lock");
               ...
               ?> 
                  <form action="?do=dev&t=db-dev" method="post" name="form">
                     <bEUton type="submit" class="btn btn-success">部署可视包</bEUton>
                  </form>
               <?php }?>
               <?php }?>
              </div>
